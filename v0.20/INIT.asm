	...
	dc.b "(C)SEGA 2014.SEP"									; Copyrght holder and release date
	dc.b "YMDJ                                           	"  ; Domestic name
	dc.b "YMDJ                                           	"  ; International name
	dc.b "GM XXXXXXXX-XX"									; Version number
	dc.w 0x0000												; Checksum
	dc.b "J               "									; I/O support
	dc.l 0x00000000											; Start address of ROM
	dc.l 0x000FFFFF												; End address of ROM
...
EntryPoint:           ; Entry point address set in ROM header
	...
	move.l #0x17, d0           ; 24 registers to write
   ...
   ; ************************************
   move.w #0x8F01, VDPCONTROLPORT    ; Set autoincrement to 1 byte
   move.w #0x93FF, VDPCONTROLPORT     ; Set bytes to fill (lo) (reg 19)
   move.w #0x94FF, VDPCONTROLPORT     ; Set bytes to fill (hi) (reg 20)
   move.w #0x9780, VDPCONTROLPORT     ; Set DMA to Fill (reg 23, bits 0-1)
   move.l #0x40000080, VDPCONTROLPORT ; Set destination address
   move.w #0x0, VDPDATAPORT           ; Value to write

   @WaitForDMA:
   move.w VDPCONTROLPORT, d1          ; Read VDP status reg
   btst   #0x1, d1                 ; Check if DMA finished
   bne.s  @WaitForDMA

   move.w #0x8F02, VDPCONTROLPORT     ; Set autoincrement to 2 bytes
   move.w #0x8ADF, VDPCONTROLPORT     ; Set H interrupt timing

   ; ************************************
   ; Clear CRAM
   ; ************************************
   move.l #VDPWRITEPALETTES, VDPCONTROLPORT ; Write to palette memory
   move.l #0x3F, d1                        ; CRAM size (in words)
   @ClrCRAM:
   move.w #0x0, VDPDATAPORT                   ; Write 0 (autoincrement is 2)
   dbra d1, @ClrCRAM
...
Main:
	jmp INITPALETTE ; Begin external main
...
VDPRegisters:
   ...
   dc.b 0x64 ; 1: Vert. interrupt on, display on, DMA on, V28 mode (40 cells vertically), + bit 2
   dc.b 0x30 ; 2: Pattern table for Scroll Plane A at 0xC000 (bits 3-5)
   dc.b 0x10 ; 3: Pattern table for Window Plane at 0x10000 (bits 1-5)
   ...
   dc.b 0x3c ; 13: Horiz. scroll table at 0xE000 (bits 0-5)
   dc.b 0x00 ; 14: Unused
   dc.b 0x02 ; 15: Autoincrement on
